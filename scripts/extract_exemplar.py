...
def save_exemplar(exemplar, platform="wechat"):
    ...
    _update_index(filename, exemplar, platform)
    return filepath


def _update_index(filename, exemplar, platform="wechat"):
...
def list_exemplars():
    ...
    for cat, entries in sorted(by_cat.items()):
        ...
        for e in entries:
            ...
            platform_tag = f"[{e.get('platform', 'wechat')}]"
            print(f"    {bar} {score:5.1f}  {platform_tag:10}  {e['source'][:40]}")
...
def main():
    ...
    parser.add_argument("--platform", "-p",
                        choices=["wechat", "xhs", "weibo", "zhihu", "toutiao", "other"],
                        default="wechat",
                        help="Source platform (for cross-platform style learning)")
    ...
    for input_path in args.inputs:
        ...
        filepath = save_exemplar(exemplar, platform=args.platform)
