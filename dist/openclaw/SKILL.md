...
> **触发条件**：首次使用时 `{baseDir}/workspace/` 目录不存在。
...
>
> 请告诉我：
> 1. 你的内容创作文件夹想放在哪里？（默认：`{baseDir}/workspace/`）
...
```
{workspace}/                          # 创作工作空间根目录
├── README.md                         # 工作空间说明（自动生成，见下方模板）
├── drafts/                           # 📝 草稿区 —— 所有未发布的初稿和草稿
│   ├── 说明：每次创作的中间产物存这里，可随时回来继续编辑
│   └── 命名规则：YYYY-MM-DD_标题_slug/
├── published/                        # 📤 已发布区 —— 已在各平台发布的最终版本
│   ├── 说明：发布后的定稿按日期归档，方便回溯和复盘
│   └── 命名规则：YYYY-MM-DD_平台_标题/
├── assets/                           # 🖼️ 素材资源 —— 图片、封面、配图、数据图表
│   ├── covers/                       # 封面图
│   ├── illustrations/                # 内文配图
│   ├── charts/                       # 数据图表
│   └── screenshots/                  # 截图/截图素材
├── research/                         # 🔬 调研笔记 —— 选题调研、竞品分析、热点追踪
│   ├── topics/                       # 选题笔记
│   ├── competitors/                  # 对标账号分析
│   └── hotspots/                     # 热点追踪记录
├── templates/                        # 📋 模板库 —— 自定义的内容模板和排版主题
│   ├── 说明：可存放你常用的文章模板、开头结尾模版等
│   └── （预置为空，随使用积累）
└── archive/                          # 📦 归档区 —— 不再活跃的历史内容
    ├── 说明：超过6个月的文章自动归档到这里
    └── （初始为空）
```
...
| 目录 | 用途 | 说明 |
|------|------|------|
| `drafts/` | 草稿区 | 所有未发布的初稿。每次创作的中间产物存这里 |
| `published/` | 已发布区 | 发布后的最终版本，按日期归档 |
| `assets/covers/` | 封面图 | 文章封面图片 |
| `assets/illustrations/` | 配图 | 内文插图 |
| `assets/charts/` | 图表 | 数据可视化图表 |
| `assets/screenshots/` | 截图 | 截图类素材 |
| `research/topics/` | 选题笔记 | 选题调研和分析记录 |
| `research/competitors/` | 对标分析 | 竞品账号分析 |
| `research/hotspots/` | 热点追踪 | 热点事件记录 |
| `templates/` | 模板库 | 自定义内容模板 |
| `archive/` | 归档区 | 超6个月历史内容 |

## 快速开始

直接说「写一篇公众号文章」即可开始创作。系统会自动：
1. 在 `drafts/` 下创建本次草稿目录
2. 完成创作后将定稿移到 `published/`
3. 相关素材存到 `assets/` 对应子目录
...
将确认的工作空间路径写入 `{baseDir}/config.yaml` 的 `workspace` 字段（不存在则追加）。
...
- `{baseDir}/style.yaml` — 风格档案（核心配置）
- `{baseDir}/references/style_manual.md` — 通用风格手册
- `{baseDir}/references/platform_styles/` — 各平台风格手册（6个）
- `{baseDir}/references/exemplars/` — 范文样本
- `{baseDir}/references/compliance.md` — 合规红线
...
| 检测 | 条件 | 路由 |
|------|------|------|
| workspace 未初始化 | `{baseDir}/workspace/` 或 config.yaml 中无 workspace 字段 | → **Init Round 1** |
| 风格档案存在 | `{baseDir}/style.yaml` 存在 | → Step 0.5 |
...
`读取: {baseDir}/references/platform_styles/`（各平台风格手册）
`读取: {baseDir}/personas/{style.yaml writing_persona}.yaml`
`读取: {baseDir}/memory/golden-sentences.md`（匹配当前话题的金句）
`读取: {baseDir}/memory/materials.md`（匹配当前话题的素材）
`读取: {baseDir}/memory/viewpoints.md`（用户相关观点）
...
- 风险提示：每篇结尾必含
- 保存到：`{workspace}/drafts/{date}_{slug}/draft.md`
...
- 如果用户确认发布，将草稿从 `drafts/` 移到 `published/`
- 相关 assets 复制到 `assets/` 对应子目录
